...
import { storeJournal } from '@/services/journalService';
import { validationSchemaStoreJournal } from '@/utils/validation';
...
export default function AddActivity({ verifyToken }) {
  ...
  const handleSubmit = async (values, actions) => {
    ...
    formData.append('userId', userData.uuid);

    toast.promise(new Promise(resolve => resolve(storeJournal(formData))),
      {
  ...
  return (
    <SecondLayout
      ...
      rightButtonIcon={<FontAwesomeIcon icon={faQuestionCircle} className="w-4 h-4 text-[#fffffe]" />}
      verifyToken={verifyToken}
    >
      <section className="mt-7">
        <Formik
          initialValues={initialValues}
          validationSchema={validationSchemaStoreJournal}
          onSubmit={handleSubmit}
        >
          {(props) => (
            <Form encType="multipart/form-data">
              <FormSelect label="Jenis Kegiatan" name="groupId" placeholder="Pilih Kegiatan" options={fieldworkOptions} />
                ...
                <label htmlFor="dropzone-file" className="relative flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600 overflow-hidden">
                  {selectedFile && (
                    <div className="absolute z-0">
                      <img src={URL.createObjectURL(selectedFile)} alt="Preview Image" className="w-full opacity-20" />
                    </div>
                  )}
